# Civitas Cargo E-Bikes - SQL migrations
...
| File | Purpose |
| --- | --- |
| `001_leads_schema.sql` | `partnership_requests` (fleet leasing / StreetShops pilot / municipality / campus / investor / manufacturing) and `contact_messages`. |
...
2. Select the Civitas Cargo application database from the left sidebar.
...
2. Set `EBIKES_BRIDGE_URL` and `EBIKES_API_KEY` in the Next.js
   deployment (Vercel) to point at the deployed `public/php` bridge and match
   the `api_key` in `config.php`.
3. Once both are set, the partnership and contact forms automatically switch
   from the fallback "backend not connected" state to live Afrihost
   persistence - no frontend code changes required.
